...
@Component({
  ...
  template: `
    @if (post()) {
      <a routerLink="/blog" class="text-sm text-muted hover:text-ink border-b border-transparent hover:border-ink">
        &larr; Back to Blog
      </a>

      <article class="mt-6">
        @if (post()!.coverImageDataUrl) {
          <img [src]="post()!.coverImageDataUrl" [alt]="post()!.title" class="w-full h-64 object-cover" />
        }
      ...
      <a routerLink="/blog" class="text-sm text-ink border-b border-ink">&larr; Back to Blog</a>
    }
  `,
})
